...
const SummaryCard = ({
  ...
  return (
    <div
      className={`rounded-xl shadow-sm border p-4 flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow ${!customColor ? 'bg-white border-gray-100' : ''}`}
      style={customColor ? {
        backgroundColor: `${customColor}08`, // 非常に薄い背景
        borderColor: `${customColor}40`,     // 薄いボーダー
      } : undefined}
    >
      <div className={`text-sm font-medium mb-1 opacity-80 text-gray-500`}>{title}</div>
      <div
      ...
      {subValue && <div className={`text-xs mt-1 opacity-70 text-gray-500`}>{subValue}</div>}
    </div>
  );
};
...
const WeaponCard = ({ name, count, color }: { name: string, count: number, color: string }) => {
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-xl border p-4 flex flex-col items-center justify-center text-center transition-all duration-300 hover:scale-[1.02] hover:shadow-lg ${isHighRarity ? 'bg-gradient-to-br from-white to-gray-50' : 'bg-white'}`}
    ...
    >
      ...
      {image && (
        <img
          src={image.src}
          alt={displayName}
          className="w-16 h-16 object-contain mb-2 drop-shadow-sm mx-auto"
        />
      )}

      <div className="font-bold text-gray-800 mb-1 truncate w-full text-sm">
        {displayName}
      </div>
      <div className="text-xl font-extrabold" style={{ color }}>
        {count.toLocaleString()}
        <span className="text-xs font-normal ml-1 text-gray-500">本</span>
      </div>
    </div>
  );
};
            ...
            <div className="flex flex-wrap gap-2">
              {gachaRates.map((rate) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-bold transition-all duration-200 ${gachaLevel === rate.level
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {[1000, 2000, 5000, 10000, 50000, 100000].map((n) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${totalPulls === n
            ...
            <div className="relative">
              <input
                ...
                min="0"
                step="1000"
                className="w-full pl-4 pr-12 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow outline-none font-mono text-lg"
                placeholder="カスタム回数"
              />
              <span className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 font-medium">回</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <SummaryCard
              ...
              subValue={`${totalRubies.toLocaleString()} ルビー`}
            />
            <SummaryCard
              title="レジェンド最上級換算"
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
              <h3 className="text-lg font-bold text-gray-800">
                最終獲得武器（期待値）
                <span className="ml-2 text-sm font-normal text-gray-500">※全て合成した場合</span>
              </h3>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <button
              ...
              className="w-full px-6 py-4 flex items-center justify-between bg-gray-50/50 hover:bg-gray-100 transition-colors text-left"
            >
            ...
            {isRateTableOpen && currentRate && (
              <div className="p-6 space-y-6 border-t border-gray-100">
                ...
                {/* Legend Table */}
                <div>
                  <div className="flex justify-between items-end mb-3 pb-2 border-b border-gray-100">
                    <span className="font-bold text-gray-700" style={{ color: rarityColors.Legend }}>Legend</span>
                    <span className="font-bold text-gray-800 text-lg">{(currentRate.legendRate * 100).toFixed(0)}%</span>
                  </div>
                      ...
                      return (
                        <div key={name} className="flex justify-between items-center text-sm py-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-gray-700">{getWeaponDisplayName(name)}</span>
                          </div>
                          <div className="text-right">
                            <span className="font-bold text-gray-800">{(actualProb * 100).toFixed(2)}%</span>
                            <span className="text-gray-400 text-xs ml-2">({(ratio * 100).toFixed(0)}%)</span>
                          </div>
                        </div>
                      );
                    })}
                ...
                {(currentRate.starRate || 0) > 0 && currentRate.starDistribution && (
                  <div>
                    <div className="flex justify-between items-end mb-3 pb-2 border-b border-gray-100 mt-2">
                      <span className="font-bold text-gray-700" style={{ color: rarityColors.Star }}>Star</span>
                      <span className="font-bold text-gray-800 text-lg">{(currentRate.starRate! * 100).toFixed(2)}%</span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2">
                      {Object.entries(currentRate.starDistribution).map(([name, ratio]) => {
                        ...
                        return (
                          <div key={name} className="flex justify-between items-center text-sm py-1">
                            ...
                            <div className="text-right">
                              <span className="font-bold text-gray-800">{(actualProb * 100).toFixed(4)}%</span>
                              <span className="text-gray-400 text-xs ml-2">({(ratio * 100).toFixed(0)}%)</span>
                            </div>
                          </div>
                        );
