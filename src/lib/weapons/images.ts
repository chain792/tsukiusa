import N4Image from '../../assets/N4.png';
import N3Image from '../../assets/N3.png';
import N2Image from '../../assets/N2.png';
import N1Image from '../../assets/N1.png';
import M4Image from '../../assets/M4.png';
import M3Image from '../../assets/M3.png';
import M2Image from '../../assets/M2.png';
import M1Image from '../../assets/M1.png';
import R4Image from '../../assets/R4.png';
import R3Image from '../../assets/R3.png';
import R2Image from '../../assets/R2.png';
import R1Image from '../../assets/R1.png';
import Unique4Image from '../../assets/Unique4.png';
import Unique3Image from '../../assets/Unique3.png';
import Unique2Image from '../../assets/Unique2.png';
import Unique1Image from '../../assets/Unique1.png';
import E4Image from '../../assets/E4.png';
import E3Image from '../../assets/E3.png';
import E2Image from '../../assets/E2.png';
import E1Image from '../../assets/E1.png';
import L4Image from '../../assets/L4.png';
import L3Image from '../../assets/L3.png';
import L2Image from '../../assets/L2.png';
import L1Image from '../../assets/L1.png';
import S4Image from '../../assets/S4.png';
import S3Image from '../../assets/S3.png';
import S2Image from '../../assets/S2.png';
import S1Image from '../../assets/S1.png';
import G4Image from '../../assets/G4.png';
import G3Image from '../../assets/G3.png';
import G2Image from '../../assets/G2.png';
import G1Image from '../../assets/G1.png';
import U4Image from '../../assets/U4.png';
import U3Image from '../../assets/U3.png';

export const weaponImages: Record<string, ImageMetadata> = {
  N4: N4Image,
  N3: N3Image,
  N2: N2Image,
  N1: N1Image,
  M4: M4Image,
  M3: M3Image,
  M2: M2Image,
  M1: M1Image,
  R4: R4Image,
  R3: R3Image,
  R2: R2Image,
  R1: R1Image,
  Unique4: Unique4Image,
  Unique3: Unique3Image,
  Unique2: Unique2Image,
  Unique1: Unique1Image,
  E4: E4Image,
  E3: E3Image,
  E2: E2Image,
  E1: E1Image,
  L4: L4Image,
  L3: L3Image,
  L2: L2Image,
  L1: L1Image,
  S4: S4Image,
  S3: S3Image,
  S2: S2Image,
  S1: S1Image,
  G4: G4Image,
  G3: G3Image,
  G2: G2Image,
  G1: G1Image,
  U4: U4Image,
  U3: U3Image,
};
